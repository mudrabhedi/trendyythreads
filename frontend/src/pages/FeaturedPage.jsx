...
export default function FeaturedPage() {
  ...
  const loadFeaturedProducts = () => {
    ...
    const featuredOnly = savedProducts.filter(
      (product) => product.featured === true && product.status !== "Hidden"
    );
  ...
  useEffect(() => {
    ...
    window.addEventListener("productsUpdated", loadFeaturedProducts);
    window.addEventListener("storage", loadFeaturedProducts);

    return () => {
      window.removeEventListener("productsUpdated", loadFeaturedProducts);
      window.removeEventListener("storage", loadFeaturedProducts);
    };
  }, []);
  ...
  const maxProductPrice = useMemo(() => {
    ...
    return Math.max(...featuredProducts.map((p) => Number(p.price || 0)));
  }, [featuredProducts]);
  ...
  const dynamicSizes = useMemo(() => {
    return allSizes
      .map((size) => {
        const count = featuredProducts.filter((product) =>
  ...
  }, [featuredProducts, priceRange, selectedSizes]);
      ...
      <div className="max-w-[1500px] mx-auto px-8">
        <div className="flex items-center justify-between border-b border-[#E5E5E5] pb-6">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold text-[#111]">
              Featured Products
            </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-10 mt-8">
          ...
          <section>
            {sortedProducts.length === 0 ? (
              <div className="min-h-[300px] flex flex-col items-center justify-center border border-dashed border-[#D9D9D9] text-gray-500">
                <p>No featured products found.</p>
                <p className="text-sm mt-2">
                  Mark products as Featured from Admin inventory.
                </p>
              </div>
            ) : (
