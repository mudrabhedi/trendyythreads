...
export default function KurtaPage() {
  ...
  const loadAdminProducts = () => {
    ...
    const kurtaOnly = savedProducts.filter(
      (product) =>
        product.category?.toLowerCase() === "kurta" &&
        product.status !== "Hidden"
    );

    setAdminProducts(kurtaOnly);
  };
  ...
  const maxProductPrice = useMemo(() => {
    if (kurtaProducts.length === 0) return 0;
    return Math.max(...kurtaProducts.map((p) => Number(p.price || 0)));
  }, [kurtaProducts]);
  ...
  const dynamicSizes = useMemo(() => {
    return allSizes
      .map((size) => {
        const count = kurtaProducts.filter((product) =>
  ...
  const filteredProducts = useMemo(() => {
    return kurtaProducts.filter((product) => {
  ...
  }, [kurtaProducts, priceRange, selectedSizes]);
    ...
    <div className="min-h-screen bg-white pt-28">
      <div className="max-w-[1500px] mx-auto px-8">
        <div className="flex items-center justify-between border-b border-[#E5E5E5] pb-6">
          <div>
            <h1 className="text-3xl font-semibold text-[#111]">Kurta</h1>
