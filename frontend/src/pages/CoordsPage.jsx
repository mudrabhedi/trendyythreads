...
export default function CoordsPage() {
  ...
  const loadAdminProducts = () => {
    ...
    const coordsOnly = savedProducts.filter(
      (product) =>
        product.category?.toLowerCase() === "coords" &&
        product.status !== "Hidden"
    );

    setAdminProducts(coordsOnly);
  };
  ...
  const maxProductPrice = useMemo(() => {
    if (coordsProducts.length === 0) return 0;
    return Math.max(...coordsProducts.map((p) => Number(p.price || 0)));
  }, [coordsProducts]);
  ...
  const dynamicSizes = useMemo(() => {
    return allSizes
      .map((size) => {
        const count = coordsProducts.filter((product) =>
  ...
  const filteredProducts = useMemo(() => {
    return coordsProducts.filter((product) => {
  ...
  }, [coordsProducts, priceRange, selectedSizes]);
    ...
    <div className="min-h-screen bg-white pt-28">
      <div className="max-w-[1500px] mx-auto px-8">
        <div className="flex items-center justify-between border-b border-[#E5E5E5] pb-6">
          <div>
            <h1 className="text-3xl font-semibold text-[#111]">Coords</h1>
