...
export default function PantsPage() {
  ...
  const loadAdminProducts = () => {
    ...
    const pantsOnly = savedProducts.filter(
      (product) =>
        product.category?.toLowerCase() === "pants" &&
        product.status !== "Hidden"
    );

    setAdminProducts(pantsOnly);
  };
  ...
  const maxProductPrice = useMemo(() => {
    if (pantsProducts.length === 0) return 0;
    return Math.max(...pantsProducts.map((p) => Number(p.price || 0)));
  }, [pantsProducts]);
  ...
  const dynamicSizes = useMemo(() => {
    return allSizes
      .map((size) => {
        const count = pantsProducts.filter((product) =>
  ...
  const filteredProducts = useMemo(() => {
    return pantsProducts.filter((product) => {
  ...
  }, [pantsProducts, priceRange, selectedSizes]);
    ...
    <div className="min-h-screen bg-white pt-28">
      <div className="max-w-[1500px] mx-auto px-8">
        <div className="flex items-center justify-between border-b border-[#E5E5E5] pb-6">
          <div>
            <h1 className="text-3xl font-semibold text-[#111]">Pants</h1>
